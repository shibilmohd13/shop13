        ...
        if "email" in request.session:
            ...
            if Cart.objects.filter(user=user, product=product_check):
                ...
                if product_check.quantity >= prod_qty:
                    ...
                    return JsonResponse(
                        {
                            "status": f"Only {str(product_check.quantity)} Quantity available"
                        }
                    )
...
def checkout(request):
    if "email" in request.session:
        ...
        cart_items = Cart.objects.filter(user=user)
        total = sum(cart_items.values_list("cart_price", flat=True))
        coupons = Coupons.objects.filter(is_active=True).filter(
    ...
    if "email" in request.session:
        ...
        if cart_items.exists():
            ...
            if balance >= total_amount_coupon:
                ...
                return JsonResponse(
                    {"success": False, "message": "You have no enough balance"}
                )
                ...
                if coupen_check.used_count < coupen_check.usage_limit:
                    ...
                    if cart_total >= coupen_check.minimum_purchase:
                        ...
                        response_data = {
                            ...
                            "discount_amount": coupen_check.discount_value,
                        }
                        ...
                        return JsonResponse(
                            {
                                "error": f"Minimum purchase amount of {round(coupen_check.minimum_purchase)} required"
                            }
                        )

                else:
                    return JsonResponse(
                        {"error": "Sorry! This code has reached its usage limit."}
                    )

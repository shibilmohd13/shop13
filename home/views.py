...
def home(request):
    obj = (
        Product.objects.prefetch_related("colorvarient_set__productimage_set")
...
def shop(request):
    ...
    context = {"obj": obj, "page_obj": page_obj, "page_count": range(page_count)}
...
def product_details(request, id):
    ...
    all_variants = (
        ColorVarient.objects.filter(product=obj.product)
        .prefetch_related("productimage_set")
        .filter(is_listed=True)
        .order_by("color")
    )
    return render(
        request, "home/details.html", {"item": obj, "all_varients": all_variants}
    )
...
def get_color_variant_details(request, id):
    try:
        ...
        image_urls = [
            img.image.url for img in color_variant.productimage_set.all()
        ]  # Get image URLs
        image_urls.reverse()
        data = {
...
def search(request):
    ...
    products_match = Product.objects.filter(combined_query, is_listed=True)
